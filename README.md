...
| Download | For |
|---|---|
| `Paint.mac-1.0.4-arm64.dmg` | Apple Silicon (M1 and later) |
| `Paint.mac-1.0.4-x64.dmg` | Intel Macs |
...
```bash
xattr -dr com.apple.quarantine ~/Downloads/Paint.mac-1.0.4-arm64.dmg
```
...
| Download | For |
|---|---|
| `Paint.mac-1.0.4-Setup-x64.exe` | Installer, 64-bit |
| `Paint.mac-1.0.4-Setup-arm64.exe` | Installer, ARM64 |
| `Paint.mac-1.0.4-portable.exe` | No install, runs from anywhere |
...
```powershell
Unblock-File -Path .\Paint.mac-1.0.4-Setup-x64.exe
```
...
| Download | For |
|---|---|
| `Paint.mac-1.0.4.AppImage` | Any distribution, x86-64 |
| `Paint.mac-1.0.4-arm64.AppImage` | Any distribution, ARM64 |
| `paint-mac_1.0.4_amd64.deb` | Debian, Ubuntu, Mint — x86-64 |
| `paint-mac_1.0.4_arm64.deb` | Debian, Ubuntu, Mint — ARM64 |

Linux has no quarantine flag — an AppImage just needs the executable bit.

```bash
# AppImage — no installation needed
chmod +x Paint.mac-1.0.4.AppImage
./Paint.mac-1.0.4.AppImage

# Debian / Ubuntu
sudo apt install ./paint-mac_1.0.4_amd64.deb
```
...
2. **Shutdown** — the app is launched four times and must actually exit each time: clean document,
   ...
   responds.
3. **Behaviour** — 265 assertions driving the real tools and document model: strokes and undo,
